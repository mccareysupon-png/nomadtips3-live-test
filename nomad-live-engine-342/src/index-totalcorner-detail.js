...
function mergeMatch(row,detail,observedAt){
  ...
  return {
    ...
    stats,
    source:{name:SOURCE_NAME,observedAt,detail:Boolean(live),mode:'DETAIL_ENABLED'},
  };
}
...
function sourceMeta(){
  return {
    ...
    detailMode:'DETAIL_ENABLED',
    detailWindow:`${DETAIL_MINUTE_FROM}-${DETAIL_MINUTE_TO}`,
    detailConcurrency:DETAIL_CONCURRENCY,
    capabilities:{today:true,minute:true,score:true,attacks:true,dangerous:true,corner:true,sot:true,off:true,detail:true},
  };
}
