...
function bookmakerCompanyId(env){
  const explicit=Number(env.GOALOO_BOOKMAKER_COMPANY_ID);
  if(Object.prototype.hasOwnProperty.call(BOOKMAKERS,explicit))return explicit;
  const name=String(env.REAL_MARKET_BOOKMAKER||'1xbet').toLowerCase();
  return name==='bet365'?8:50;
}
...
// Goaloo runOddsData AH row is interpreted as:
// HOME raw odds | HOME-perspective Asian line | AWAY raw odds.
// The line sign is never inferred from odds. HOME keeps the source sign; AWAY
// gets the exact inverse only when the detector evaluates the AWAY selection.
...
export function parseGoalooRunOdds(source,providerCompanyId=50){
  ...
  for(const raw of String(source||'').split('$')){
    ...
    const record={
      ...
      asianHandicap:ah.length>=3&&number(ah[1])!==null?{home:marketOdd(ah[0],'AH'),line:number(ah[1]),away:marketOdd(ah[2],'AH'),linePerspective:'HOME',raw:{home:ah[0],line:ah[1],away:ah[2]}}:null,
      overUnder:ou.length>=3?{over:marketOdd(ou[0],'OU'),line:number(ou[1]),under:marketOdd(ou[2],'OU'),raw:{over:ou[0],line:ou[1],under:ou[2]}}:null,
      providerCompanyId:Number(providerCompanyId)||50,
      providerName:bookmakerName(Number(providerCompanyId)||50)
    };
...
function parseDetailInStats(source,allowedIds=null){
  ...
  for(const m of String(source||'').matchAll(assignment)){
    ...
    for(const row of m[2].matchAll(rowRe)){
      const key=DETAIL_IN_CORE_MAP[Number(row[1])];if(!key)continue;
...
function mergeCoreStats(match,detailStats){
  const stats={...(match?.stats||{})},applied=[];
  for(const key of CORE_STATS_KEYS){
    const pair=detailStats?.[key];if(!pair)continue;
    const home=number(pair.home),away=number(pair.away);if(home===null||away===null)continue;
    stats[key]={home,away};applied.push(key);
  }
  match.stats=stats;
  match.coreStatsComplete=coreStatsComplete(stats);
  ...
  return{applied,structuredComplete:applied.length===CORE_STATS_KEYS.length};
}

async function sourceText(url,seconds){
  const bucket=Math.floor(Date.now()/(seconds*1000));
  const response=await fetch(`${url}?t=${bucket}`,{
    headers:{'user-agent':'NOMADTIPS3-CAR3.4-GoalooOnly/1.0','accept':'*/*','accept-language':'en-US,en;q=0.8'},
...
function pressure(stats,w){
  let h=0,a=0;
  for(const [k,wt] of Object.entries(w||{})){h+=(number(stats?.[k]?.home)||0)*wt;a+=(number(stats?.[k]?.away)||0)*wt;}
  const t=Math.max(.0001,h+a);return{home:Math.round(h/t*100),away:Math.round(a/t*100)};
}
...
function evaluateGoalooAh(match,config,snapshots){
  ...
  const base=baselineFor(match.sourceMatchId,side,snapshots,config,match.stats);
  const cur={dangerous:sidePair(match.stats?.dangerous_attacks,side).selected,shots:sidePair(match.stats?.shots,side).selected,sot:sidePair(match.stats?.shots_on_target,side).selected,corners:sidePair(match.stats?.corners,side).selected};
  ...
  const passed=evRules.filter(r=>r[1]>=r[2]).length;
  const required=config.attackEvidenceRequirement==='ALL'?evRules.length:Number(config.attackEvidenceRequirement);
  const score=sidePair(match.score,side),red=sidePair(match.stats?.red_cards,side),goalGap=Math.abs(score.selected-score.opponent);
  const rawHomeLine=number(match.odds?.asianHandicap?.line);
  const selectedLine=rawHomeLine===null?null:(side==='AWAY'?-rawHomeLine:rawHomeLine);
  const odds=number(match.odds?.asianHandicap?.[side==='AWAY'?'away':'home']);
  const oddsOk=odds!==null&&odds>=config.oddsMin&&(config.oddsMax===null||odds<=config.oddsMax);
  const ahOk=selectedLine!==null&&selectedLine>=config.ahMin&&(config.ahMax===null||selectedLine<=config.ahMax);
  const redOk=config.redCardPolicy==='ALLOW'||(config.redCardPolicy==='REJECT_SELECTED'?red.selected===0:(red.selected===0&&red.opponent===0));
  const marketState=match.realMarket?.status||'NO_AH';
  const gates=[
    ...
    ['REAL MARKET',marketState==='MATCH',marketState],
    ['REAL PRICE AGE',marketState==='MATCH',marketState==='MATCH'?'Goaloo current cycle':'n/a'],
    ['MARKET / ODDS',oddsOk&&ahOk,odds===null?'waiting Goaloo AH':`AH ${selectedLine>=0?'+':''}${selectedLine} @ ${odds}`],
    ['MOMENTUM',selMomentum>=config.momentumMin,`${selMomentum}% / ≥${config.momentumMin}%`],
    ['EVIDENCE',!config.attackEvidenceEnabled||passed>=required,`${passed}/${evRules.length} · need ${config.attackEvidenceRequirement}`],
...
export class Car31State extends MainV3Car31State{
  async scan(trigger='cron'){
    ...
    const baseResponse=await BaseCar31State.prototype.scan.call(this,trigger);
    if(!baseResponse.ok)return baseResponse;
    const basePayload=await baseResponse.clone().json().catch(()=>({}));
    const at=basePayload.generatedAt||new Date().toISOString();
    const companyId=bookmakerCompanyId(this.env),bookmaker=bookmakerName(companyId),providerSource=oddsSource(companyId);
    try{
      const [oddsResult,detailResult,configResponse]=await Promise.all([
        sourceText(providerSource,ENRICH_SECONDS).then(value=>({ok:true,value})).catch(error=>({ok:false,error:String(error?.message||error)})),
        ...
        BaseCar31State.prototype.fetch.call(this,new Request('https://car34.internal/config'))
      ]);
      const configPayload=await configResponse.json();
      const config={...configPayload.config,market:'AH',engineEnabled:configPayload.config?.engineEnabled!==false,confirmationRounds:1};
      const latest=await this.state.storage.get('latest')||{generatedAt:at,matches:[]};
      const snapshots=await this.state.storage.get('snapshots')||[];
      const confirmationStreaks=await this.state.storage.get('confirmationStreaksV34')||{};
      const history=await this.state.storage.get('history')||[];
      const ids=new Set((latest.matches||[]).map(m=>String(m.sourceMatchId)));
      const oddsMap=oddsResult.ok?parseGoalooRunOdds(oddsResult.value,companyId):new Map();
      const detailStatsMap=detailResult.ok?parseDetailInStats(detailResult.value,ids):new Map();
      ...
      for(const match of latest.matches||[]){
        ...
        const liveOdds=oddsMap.get(id),ah=liveOdds?.asianHandicap||null;
        if(liveOdds)oddsMatched++;
        if(ah&&number(ah.line)!==null&&number(ah.home)!==null&&number(ah.away)!==null){
          ahMatched++;
          match.odds={...(match.odds||{}),asianHandicap:{...ah,updatedAt:at,provider:bookmaker,providerCompanyId:companyId,linePerspective:'HOME'}};
          match.realMarket={source:'Goaloo',bookmaker,status:'MATCH',checkedAt:at,oddsUpdatedAt:at,marketAgeSeconds:0,feed:`runOddsData_${companyId}`,linePerspective:'HOME',mapping:'GOALOO_MATCH_ID'};
          match.currentAh={status:'MATCH',line:ah.line,homeOdds:ah.home,awayOdds:ah.away,updatedAt:at,provider:`${bookmaker} · Goaloo`,marketAgeSeconds:0,linePerspective:'HOME'};
        }else{
          match.odds={...(match.odds||{}),asianHandicap:null};
          match.realMarket={source:'Goaloo',bookmaker,status:oddsResult.ok?'NO_AH':'ERROR',checkedAt:at,oddsUpdatedAt:null,marketAgeSeconds:null,feed:`runOddsData_${companyId}`,linePerspective:'HOME',mapping:'GOALOO_MATCH_ID',error:oddsResult.ok?null:oddsResult.error};
          match.currentAh={status:oddsResult.ok?'NO_AH':'ERROR',provider:`${bookmaker} · Goaloo`,updatedAt:null,marketAgeSeconds:null,linePerspective:'HOME'};
        }
      }
      ...
      for(const match of latest.matches||[]){
        const engine=evaluateGoalooAh(match,config,snapshots),key=`${match.sourceMatchId}:${engine.side}:AH`;
        ...
        if(!existing&&!dailyBlocked&&confirmationStreaks[key]>=config.confirmationRounds){
          ...
          history.push({key,id:match.sourceMatchId,selectionDate:today,selectedAt:at,league:match.league,home:match.home,away:match.away,selectedSide:engine.side,selectedTeam,entryMinute:match.minute,entryScore:{...match.score},market:'AH',line:engine.selectedLine,rawLine:engine.rawLine,selectedLine:engine.selectedLine,linePerspective:'SELECTED',odds:engine.odds,bookmaker,bookmakerCompanyId:companyId,pricingSource:`GOALOO_RUNODDS_${companyId}`,oddsUpdatedAt:at,marketAgeSeconds:0,goalooLinePerspective:'HOME',momentum:engine.momentum,evidence:engine.evidence,kickoffUtc:match.kickoffUtc,status:'PENDING',ftStatus:null,settledAt:null,finalScore:null,result:'PENDING'});
          newCount++;
        }
        match.engine={...engine,market:'AH',streak:confirmationStreaks[key]||0,dailyBlocked,bookmaker,bookmakerCompanyId:companyId};
        match.enrichment={...(match.enrichment||{}),odds:match.realMarket?.status==='MATCH'?'GOALOO_DIRECT':'NONE',bookmaker,bookmakerCompanyId:companyId,coreStats:match.coreStatsProvenance||'BASE'};
      }
      ...
      latest.sourceMode='GOALOO_ONLY_STATS_PLUS_AH';
      latest.market='AH';
      latest.engineEnabled=config.engineEnabled;
      latest.realMarketPipe={status:config.engineEnabled?(oddsResult.ok?'DIRECT':'ERROR'):'PAUSED',source:'Goaloo',bookmaker,bookmakerCompanyId:companyId,feed:`runOddsData_${companyId}`,transport:'GOALOO_DIRECT',api:'NONE',oddsMatched,ahMatched,matchCount,engineEnabled:config.engineEnabled,linePerspective:'HOME',error:oddsResult.ok?null:oddsResult.error,at};
      latest.coreStatsPipe={status:detailResult.ok?'DIRECT':'ERROR',source:'Goaloo',feed:'detailIn.js',detailMatched,filledMatches:detailAppliedMatches,structuredCompleteMatches,coreStatsReady,matchCount,error:detailResult.ok?null:detailResult.error,at};
      latest.cardAhPipe={source:'Goaloo',bookmaker,feed:`runOddsData_${companyId}`,transport:'GOALOO_DIRECT',visibleCards:(latest.matches||[]).filter(m=>m.realMarket?.status==='MATCH'||m.engine?.decision==='NEAR'||Number(m.engine?.streak||0)>0).length,at};

      await this.state.storage.put('latest',latest);
      await this.state.storage.put('history',history);
      await this.state.storage.put('confirmationStreaksV34',confirmationStreaks);
      await this.state.storage.put('realMarketPipe',latest.realMarketPipe);
      await this.state.storage.put('coreStatsPipe',latest.coreStatsPipe);
      await this.state.storage.put('settlementContract',{version:'BET365_V4',reference:'BET365_FOOTBALL_RULES',settlement:'market_aware_live',asianHandicap:'post_entry_score',asianHandicapLinePerspective:'selected_team',quarterLines:'split_stake_50_50',updatedAt:at});
      ...
      const pipe={status:'ERROR',source:'Goaloo',transport:'GOALOO_DIRECT',api:'NONE',error:String(error?.message||error),at};
      await this.state.storage.put('realMarketPipe',pipe);
      return json({ok:false,error:pipe.error,realMarketPipe:pipe},502);
    }
  }

  async health(){
    const base=await BaseCar31State.prototype.health.call(this);
    const realMarketPipe=await this.state.storage.get('realMarketPipe')||null,coreStatsPipe=await this.state.storage.get('coreStatsPipe')||null;
    return{...base,engine:'CAR 3.4 REAL MARKET AUDIT',sourceMode:'GOALOO_ONLY_STATS_PLUS_AH',market:'AH',cron:'EVERY_2_MINUTES',realMarketPipe,coreStatsPipe};
  }
}
