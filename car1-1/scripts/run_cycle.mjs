...
function nextCutoffUtc(date){
  ...
  // Thailand is UTC+7 with no DST. Next local 08:00 equals next-day 01:00 UTC.
...
function parseOdds(text){
  ...
  if(m)return {home:+m[5],draw:+m[6],away:+m[7]};
  const i=block.match(/Bet365\s+Initial\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)/i);
  if(i)return {home:+i[1],draw:+i[2],away:+i[3]};
  const any=block.match(/(?:Crown|Sbobet)\s+Initial\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)([\s\S]{0,500}?)\bLive\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)/i);
  if(any)return {home:+any[5],draw:+any[6],away:+any[7]};
  const anyInitial=block.match(/(?:Crown|Sbobet)\s+Initial\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.]+)/i);
  if(anyInitial)return {home:+anyInitial[1],draw:+anyInitial[2],away:+anyInitial[3]};
  return null;
}
...
function predictedScore(side,homeVenue,awayVenue){
  const selected=side==='home'?homeVenue:awayVenue;
  const opponent=side==='home'?awayVenue:homeVenue;
  const sg=Math.max(.4,(selected.gfpg+opponent.gapg)/2);
  const og=Math.max(.2,(opponent.gfpg+selected.gapg)/2);
  let s=Math.min(4,Math.max(1,Math.round(sg))),o=Math.min(3,Math.max(0,Math.round(og)));
  if(s<=o)s=Math.min(4,o+1);
  return side==='home'?`${s}-${o}`:`${o}-${s}`;
}
...
async function main(){
  ...
  const matches=await feed.evaluate(()=>[...document.querySelectorAll('tr[id^="tr1_"]')].map(tr=>{
    ...
    const cells=[...tr.querySelectorAll('td')],time=cells[2];
  ...
  const threshold=Date.now()+MIN_LEAD_MINUTES*60*1000;
  const cutoff=nextCutoffUtc(now).getTime();
  const upcoming=matches.map(m=>({...m,kickoffUtc:m.kickoffRaw?new Date(m.kickoffRaw.replace(' ','T')+'Z').toISOString():null}))
    .filter(m=>m.home&&m.away&&m.kickoffUtc&&new Date(m.kickoffUtc).getTime()>=threshold&&new Date(m.kickoffUtc).getTime()<=cutoff&&!/\d+\s*-\s*\d+/.test(m.score)&&!/^\d+'?$/.test(m.status))
  ...
  async function worker(){
    ...
    while(true){
      ...
      try{
        ...
        const ho=metrics(recent.homeRows,m.home,null),ao=metrics(recent.awayRows,m.away,null),hv=metrics(recent.homeRows,m.home,'home'),av=metrics(recent.awayRows,m.away,'away');
        if(!ho||!ao||!hv||!av||Math.min(ho.sample,ao.sample,hv.sample,av.sample)<MIN_SAMPLE)continue;
        const strength=(ho.ppg-ao.ppg)*.34+(hv.ppg-av.ppg)*.36+(ho.gdpg-ao.gdpg)*.18;
        const side=strength>0?'home':'away';
        const absStrength=Math.abs(strength);
        const overallEdge=side==='home'?ho.ppg-ao.ppg:ao.ppg-ho.ppg;
        const venueEdge=side==='home'?hv.ppg-av.ppg:av.ppg-hv.ppg;
  ...
  selected.forEach((x,i)=>console.log(`${i+1}. ${x.pick} @ ${x.odds} confidence=${x.confidence}% strength=${x.analysis.absoluteStrength} overallEdge=${x.analysis.overallPpgEdge} venueEdge=${x.analysis.venuePpgEdge} (${x.home} vs ${x.away})`));
}
