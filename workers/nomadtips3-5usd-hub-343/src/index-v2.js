const VERSION='nomad343-5usd-hub-v5-rate-telemetry';
...
function headers(request){const origin=request.headers.get('origin')||'';const h=new Headers({'content-type':'application/json; charset=utf-8','cache-control':'no-store','access-control-allow-methods':'GET,OPTIONS','access-control-allow-headers':'content-type',vary:'Origin'});h.set('access-control-allow-origin',ALLOWED_ORIGINS.has(origin)?origin:'https://www.nomadtips3.com');return h}
...
function providerRate(response,startedAt){
  const limit=finite(response.headers.get('x-ratelimit-limit')??response.headers.get('x-ratelimit-requests-limit'));
  const remaining=finite(response.headers.get('x-ratelimit-remaining')??response.headers.get('x-ratelimit-requests-remaining'));
  const reset=finite(response.headers.get('x-ratelimit-reset'));
  const retryAfterSec=finite(response.headers.get('retry-after'));
  return {limit,remaining,reset,retryAfterSec,observedAt:now(),status:response.status,durationMs:Math.max(0,now()-startedAt),headerAvailable:limit!==null||remaining!==null||reset!==null};
}
async function fetchJson(url,key){const ac=new AbortController(),timer=setTimeout(()=>ac.abort(),TIMEOUT_MS),startedAt=now();try{const r=await fetch(url,{cache:'no-store',signal:ac.signal,headers:{accept:'application/json',authorization:`Bearer ${key}`}});const rate=providerRate(r,startedAt),raw=await r.text();let j=null;try{j=JSON.parse(raw)}catch{}if(!r.ok){const e=new Error(`provider:HTTP_${r.status}`);e.status=r.status;e.retryAfter=rate.retryAfterSec;e.rate=rate;throw e}if(!j||typeof j!=='object'){const e=new Error('provider:INVALID_JSON');e.status=r.status;e.rate=rate;throw e}return {json:j,rate}}catch(e){if(e?.name==='AbortError'){const x=new Error('provider:TIMEOUT');x.status=0;throw x}throw e}finally{clearTimeout(timer)}}
async function fetchPages(env,query,perPage,maxPages){if(!env.FIVEDOLLAR_API_KEY)throw new Error('provider:FIVEDOLLAR_API_KEY_MISSING');const rows=[];let page=1,hasMore=false,lastRate=null;do{const join=query?`&${query}`:'';const out=await fetchJson(`${API_BASE}/fixtures?per_page=${perPage}&page=${page}${join}`,env.FIVEDOLLAR_API_KEY);const j=out.json;lastRate=out.rate;rows.push(...extract(j));hasMore=pageInfo(j).hasMore;page+=1}while(hasMore&&page<=maxPages);return {rows,requests:page-1,guardHit:hasMore&&page>maxPages,lastRate}}
...
function knownRequestsPerMinute(meta){const perRefresh=finite(meta?.providerRequestCount)??0;return Math.round(perRefresh*(60_000/REFRESH_MS)*10)/10}
function usedInWindow(rate){const limit=finite(rate?.limit),remaining=finite(rate?.remaining);return limit!==null&&remaining!==null?Math.max(0,limit-remaining):null}
...
export class FiveUsdHub{
  ...
  async state(){return await this.ctx.storage.get('state')||{lastAttemptAt:null,lastSuccessAt:null,lastError:null,lastRateLimitAt:null,retryAfterSec:null,providerRate:null}}
  ...
  async refresh(){const attempt=now(),oldMeta=await this.meta(),oldRows=await this.readRows(oldMeta),oldState=await this.state();await this.ctx.storage.put('state',{...oldState,lastAttemptAt:attempt});try{const p=await fetchProvider(this.env);const map=new Map();for(const raw of p.today.rows){const n=normalize(raw);if(n.fixtureId)map.set(n.fixtureId,n)}for(const old of oldRows){if(map.has(old.fixtureId))map.set(old.fixtureId,merge(old,map.get(old.fixtureId)))}for(const raw of p.live.rows){const n=normalize(raw);if(n.fixtureId)map.set(n.fixtureId,merge(map.get(n.fixtureId),n))}const rows=[...map.values()].sort((a,b)=>(a.kickoffAt??0)-(b.kickoffAt??0));const dataChunks=chunks(rows),snapshotId=attempt.toString(36);for(let i=0;i<dataChunks.length;i++)await this.ctx.storage.put(`snapshot:${snapshotId}:${i}`,JSON.stringify(dataChunks[i]));const counts={scheduled:rows.filter(x=>x.boardState==='scheduled').length,live:rows.filter(x=>x.boardState==='live').length,finished:rows.filter(x=>x.boardState==='finished').length,unknown:rows.filter(x=>x.boardState==='unknown').length};const meta={version:VERSION,fetchedAt:now(),snapshotId,chunkCount:dataChunks.length,fixtureCount:rows.length,counts,providerRequestCount:p.today.requests+p.live.requests,providerRequestBudget:p.requestBudget,todayRequests:p.today.requests,liveRequests:p.live.requests,guardHit:Boolean(p.today.guardHit||p.live.guardHit),include:p.include,todayWindow:{start:p.window.start,end:p.window.end,timeZone:'Asia/Bangkok'}};await this.ctx.storage.put('meta',meta);await this.ctx.storage.put('state',{...oldState,lastAttemptAt:attempt,lastSuccessAt:meta.fetchedAt,lastError:null,retryAfterSec:null,providerRate:p.rate??oldState.providerRate??null});if(oldMeta?.snapshotId&&oldMeta.snapshotId!==snapshotId){for(let i=0;i<Number(oldMeta.chunkCount||0);i++)await this.ctx.storage.delete(`snapshot:${oldMeta.snapshotId}:${i}`)}return meta}catch(e){const status=Number(e?.status||0),retryAfter=finite(e?.retryAfter??e?.rate?.retryAfterSec);await this.ctx.storage.put('state',{...oldState,lastAttemptAt:attempt,lastError:String(e?.message||e),lastRateLimitAt:status===429?attempt:oldState.lastRateLimitAt??null,retryAfterSec:status===429?(retryAfter??10):oldState.retryAfterSec??null,providerRate:e?.rate??oldState.providerRate??null});if(oldMeta)return oldMeta;throw e}}
  async snapshot(){let m=await this.meta();if(!m||!m.fetchedAt||now()-m.fetchedAt>=REFRESH_MS){try{m=await this.refreshIfDue()}catch{}}m=await this.meta();const s=await this.state();if(!m)return {ok:false,version:VERSION,error:s.lastError||'NO_SNAPSHOT'};const rows=await this.readRows(m),ageMs=Math.max(0,now()-m.fetchedAt),rate=s.providerRate??null;return {ok:true,version:VERSION,provider:'5DollarFootballAPI',fetchedAt:m.fetchedAt,ageMs,stale:ageMs>STALE_MS,fixtureCount:rows.length,counts:m.counts,providerRequestCount:m.providerRequestCount,providerRequestBudget:m.providerRequestBudget,todayRequests:m.todayRequests,liveRequests:m.liveRequests,guardHit:m.guardHit,include:m.include,todayWindow:m.todayWindow,lastAttemptAt:s.lastAttemptAt,lastSuccessAt:s.lastSuccessAt,lastError:s.lastError,lastRateLimitAt:s.lastRateLimitAt??null,retryAfterSec:s.retryAfterSec??null,providerRate:rate,providerUsedInWindow:usedInWindow(rate),estimatedKnownRequestsPerMinute:knownRequestsPerMinute(m),fixtures:rows}}
  async health(){const m=await this.meta(),s=await this.state(),ageMs=m?.fetchedAt?Math.max(0,now()-m.fetchedAt):null,rate=s.providerRate??null,observedAt=finite(rate?.observedAt),providerLimit=finite(rate?.limit),providerRemaining=finite(rate?.remaining);return {ok:Boolean(m),component:'5USD_HUB',version:VERSION,fixtureCount:m?.fixtureCount??0,counts:m?.counts??{scheduled:0,live:0,finished:0,unknown:0},fetchedAt:m?.fetchedAt??null,ageMs,stale:ageMs===null||ageMs>STALE_MS,refreshMs:REFRESH_MS,providerRequestCount:m?.providerRequestCount??0,providerRequestBudget:m?.providerRequestBudget??MAX_PROVIDER_REQUESTS_PER_REFRESH,estimatedKnownRequestsPerMinute:knownRequestsPerMinute(m),accountLimitPerMinute:providerLimit??ACCOUNT_LIMIT_PER_MINUTE,providerRateLimit:providerLimit,providerRateRemaining:providerRemaining,providerRateReset:finite(rate?.reset),providerRateObservedAt:observedAt,providerRateAgeMs:observedAt===null?null:Math.max(0,now()-observedAt),providerRateStatus:finite(rate?.status),providerLatencyMs:finite(rate?.durationMs),providerHeadersAvailable:Boolean(rate?.headerAvailable),providerUsedInWindow:usedInWindow(rate),include:m?.include??null,lastAttemptAt:s.lastAttemptAt??null,lastSuccessAt:s.lastSuccessAt??null,lastError:s.lastError,lastRateLimitAt:s.lastRateLimitAt??null,retryAfterSec:s.retryAfterSec??null}}
...
export default{async fetch(request,env){if(request.method==='OPTIONS')return json(request,null,204);const u=new URL(request.url),s=stub(env);if(request.method==='GET'&&u.pathname==='/snapshot'){const r=await s.fetch('https://hub.internal/snapshot');return json(request,await r.json(),r.status)}if(request.method==='GET'&&(u.pathname==='/health'||u.pathname==='/status')){const r=await s.fetch('https://hub.internal/health');return json(request,await r.json(),r.status)}return json(request,{ok:false,version:VERSION,error:'NOT_FOUND'},404)},async scheduled(_event,env,ctx){ctx.waitUntil(stub(env).fetch('https://hub.internal/_internal/refresh',{method:'POST'}))}}