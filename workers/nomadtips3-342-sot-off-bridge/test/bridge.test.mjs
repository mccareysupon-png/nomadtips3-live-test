...
import {chooseGoaloo,enrichFeed,teamScore} from '../src/index.js';
...
const g=(over={})=>({sourceMatchId:'g1',league:'Hong Kong Premier League',home:'Hong Kong FC',away:'LeeMan',minute:45,score:{home:0,away:1},stats:{shots_on_target:{home:4,away:2},shots_off_target:{home:3,away:5}},...over});
...
test('strict matcher accepts same orientation score and close minute',()=>{
  const pick=chooseGoaloo(tc(),[g()]);
  assert.equal(pick.reason,'MATCHED');
  assert.equal(pick.match.sourceMatchId,'g1');
});

test('strict matcher rejects score mismatch',()=>{
  const pick=chooseGoaloo(tc(),[g({score:{home:1,away:0}})]);
  assert.equal(pick.match,null);
});

test('strict matcher rejects reversed HOME AWAY',()=>{
  const pick=chooseGoaloo(tc(),[g({home:'Lee Man FC',away:'Hong Kong FC',score:{home:1,away:0}})]);
  assert.equal(pick.match,null);
});

test('strict matcher rejects minute gap over five',()=>{
  const pick=chooseGoaloo(tc(),[g({minute:51})]);
  assert.equal(pick.match,null);
});

test('bridge only fills SOT OFF and preserves TotalCorner-owned fields',()=>{
  const input={ok:true,matches:[tc()]};
  const out=enrichFeed(input,{ok:true,observedAt:2000,matches:[g()]},2000);
...
test('missing Shot Off remains null and is never fabricated',()=>{
  const goaloo=g({stats:{shots_on_target:{home:4,away:2},shots_off_target:null}});
  const out=enrichFeed({ok:true,matches:[tc({id:'tc2'})]},{ok:true,observedAt:2000,matches:[goaloo]},2000);
...
test('bridge retains prior minute SOT OFF history for rolling deltas',()=>{
  ...
  enrichFeed({ok:true,matches:[m44]},{ok:true,observedAt:1000,matches:[g({minute:44,stats:{shots_on_target:{home:2,away:1},shots_off_target:{home:1,away:4}}})]},1000);
  const m45=tc({id:'tc-history'});
  const out=enrichFeed({ok:true,matches:[m45]},{ok:true,observedAt:2000,matches:[g()]},2000);
