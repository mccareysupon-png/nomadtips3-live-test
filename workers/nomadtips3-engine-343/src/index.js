...
function oddsRoot(payload){
  ...
  return payload?.data?.odds??payload?.odds??payload?.data??payload??null;
}
function fixtureOddsRoot(f){return oddsRoot(f?.providerOdds??f?.odds??f?.markets??null)}
    ...
    try{
      ...
      for(const f of hub.fixtures||[]){
        const analysis={};if(isLive(f)){
          ...
          if(marketCandidates.length)fixtureCandidates.push({fixture:f,candidates:marketCandidates});
        }
        board.push({...f,analysis});
      }
      const refereeErrors=[];
      for(const item of fixtureCandidates){
        const f=item.fixture,id=String(f.fixtureId),root=fixtureOddsRoot(f),b=board.find(x=>String(x.fixtureId)===id);
        if(!root){for(const c of item.candidates)if(b)b.analysis[c.market]={state:'PRICE_DATA_UNAVAILABLE'};refereeErrors.push({fixtureId:id,error:'HUB_BATCH_ODDS_UNAVAILABLE'});continue}
        ...
        for(const [key,cands] of grouped){
          const best=pickBestPriced(cands,root,f,settings);if(!best){if(b)b.analysis[key]={state:'NO_PRICE_PASS'};continue}
      ...
      await this.ctx.storage.put('histories',histories);await this.ctx.storage.put('signals',signals.slice(-MAX_SIGNALS));await this.ctx.storage.put('board',{ok:true,version:VERSION,hubVersion:hub.version,hubFetchedAt:hub.fetchedAt,hubAgeMs:hub.ageMs,stale:hub.stale,counts:hub.counts,fixtures:board,runState:run,referee:{source:'HUB_BATCH',externalRequests:0,requests:0,queued:0,errors:refereeErrors}});
      const meta={ok:true,startedAt,finishedAt:now(),fixtureCount:board.length,liveCount:board.filter(isLive).length,signalCount:signals.filter(s=>s.status==='PENDING').length,unresolvedCount:signals.filter(s=>s.status==='UNRESOLVED').length,reconciled,refereeRequests:0,refereeQueued:0,lastError:null};await this.ctx.storage.put('lastScan',meta);return meta;
