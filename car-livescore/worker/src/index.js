...
function refs(text){
  const out=[];
  const patterns=[/(?:wss?|https?):\/\/[^"'\s)]+/gi,/['"](\/[^'"\s]{3,160})['"]/g];
  for(const re of patterns){let m;while((m=re.exec(text))&&out.length<160){const v=(m[1]||m[0]).replace(/\\u0026/g,'&');if(/(ws|socket|score|soccer|football|data|api|live|signal|push|match|bf_|detail)/i.test(v))out.push(v);}}
  return [...new Set(out)].slice(0,100);
}
function snippets(text){
  const out=[]; const re=/.{0,100}(WebSocket|wss?:|signalr|socket|push|score|liveData|matchData|apiUrl|serverUrl).{0,180}/gi; let m;
  while((m=re.exec(text))&&out.length<35)out.push(m[0].replace(/\s+/g,' ').slice(0,320));
  return [...new Set(out)];
}
async function probe(){
  ...
  for(const path of INSPECT){
    try{const r=await get(BASE+path);inspected.push({path,status:r.status,type:r.type,length:r.length,refs:refs(r.text),snippets:snippets(r.text)});}catch(e){inspected.push({path,error:String(e?.message||e)});}
  }
  return {ok:true,service:'CAR LIVESCORE NOWGOAL PROBE',source:BASE,generatedAt:new Date().toISOString(),home:{status:home.status,type:home.type,length:home.length},scripts:scriptUrls.slice(0,80),candidates,inspected};
}
