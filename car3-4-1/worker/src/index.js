...
async function sourceProbe(env){
  ...
  return {
    ok:Boolean(discovery.sourceUrl),
    provider:'GOALOO',
    sourceUrl:discovery.sourceUrl,
    indexMatches:discovery.matchCount,
    liveMatches:discovery.live.length,
    leagueCount:discovery.leagueCount,
...
async function liveSnapshot(env){
  ...
  if(!discovery.sourceUrl){
    return {status:502,body:{ok:false,generatedAt:collectedAt,phase:'SOURCE_MONITOR',source:'GOALOO',error:'GOALOO_INDEX_UNAVAILABLE',errors:discovery.errors,matches:[]}};
  }
  const hydrated=await hydrateLive(env,discovery.live,collectedAt);
  const matches=hydrated.map(normalizeGoalooMatch);
  const quality=summarizeQuality(matches);
  return {
    status:200,
    body:{
      ok:true,
      ...
      source:'GOALOO',
      sourceUrl:discovery.sourceUrl,
      index:{matches:discovery.matchCount,live:discovery.live.length,leagues:discovery.leagueCount},
      quality,
      errors:discovery.errors,
      elapsedMs:Date.now()-started,
      matches
  ...
  async fetch(request,env){
    ...
    try{
      if(url.pathname==='/health'){
        return json({ok:true,service:'nomadtips3-car341-source-monitor',...phaseConfig(),now:new Date().toISOString()},200,requestId);
      }
      ...
      return json({
        ok:true,
        service:'nomadtips3-car341-source-monitor',
        version:'3.4.1',
        phase:'SOURCE_MONITOR',
        routes:['GET /health','GET /config','GET /source-health','GET /live'],
        note:'Pricing, detector, persistence, settlement and cron are intentionally disconnected.'
      },200,requestId);
    }catch(error){
