...
test('final detector inverts away line perspective when needed',()=>{const cfg=normalizeConfig({side:'HOME',momentumMin:50,ahMin:1,oddsMin:1.4});const r=evaluateFinal(match,cfg,[baseline],{status:'MATCH',matchConfidence:.95,marketAgeSeconds:10,ah:{line:1,home:1.55,away:2.2,bookmaker:'1xbet',updatedAt:new Date().toISOString()}});assert.equal(r.decision,'SIGNAL');assert.equal(r.selectedLine,1);assert.equal(r.odds,1.55);});
