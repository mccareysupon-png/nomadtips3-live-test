...
test('gate 1: Nowgoal exposes all 18 approved bookmaker identities with unique company/source IDs',()=>{
  assert.equal(NOWGOAL_BOOKMAKERS.length,18);
  assert.equal(new Set(NOWGOAL_BOOKMAKERS.map(item=>item.companyId)).size,18);
  assert.equal(new Set(NOWGOAL_BOOKMAKERS.map(item=>item.sourceId)).size,18);
...
test('gate 2: retired SOURCE 8 stays absent while legacy and TotalCorner sockets remain registered',()=>{
  ...
  for(let position=9;position<=23;position++) assert.equal(ids.includes(`source${position}`),true);
});
...
test('gate 5: one Nowgoal session reads all extra change feeds without requiring extra full-feed requests',async()=>{
  ...
  assert.equal(result.bookmakers.total,18);
  assert.equal(result.bookmakers.extraChecked,15);
...
test('gate 7: price snapshots expose every Nowgoal bookmaker while preserving verified-bookmaker fail closed',()=>{
  ...
  source5.nowgoalPeers={source6:ready('Nowgoal','Bet365',-.5,1.84),source19:ready('Nowgoal','Sbobet',-.5,1.83)};
  ...
  assert.equal(snapshots.filter(item=>item.source==='Nowgoal').length,18);
