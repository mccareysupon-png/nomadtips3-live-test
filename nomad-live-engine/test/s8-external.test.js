...
test('S1-S7 identities remain intact and S8 is explicitly shadow-only',()=>{
  assert.deepEqual(
  ...
  const s8=PRICE_SOURCE_REGISTRY.find(item=>item.id==='source8');
  assert.equal(s8.selectable,false);
  assert.equal(s8.shadowOnly,true);
});

test('S8 PASS is visible but can never beat a legacy primary',()=>{
...
test('S8 outage never blocks an existing primary',()=>{
