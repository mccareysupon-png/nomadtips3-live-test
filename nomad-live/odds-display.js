...
const receiverOnly=()=>document.body?.dataset?.oddsRole==='receiver'||document.documentElement?.dataset?.oddsRole==='receiver';
...
function mount(){if(receiverOnly()||document.querySelector('.odds-display-control'))return;const search=document.querySelector('.toolbar .search');if(search){search.insertAdjacentHTML('beforebegin',markup());return}const panel=document.querySelector('.summary-grid + .panel');if(panel){const s=document.createElement('section');s.className='odds-display-toolbar';s.innerHTML=markup();panel.parentNode.insertBefore(s,panel)}}
