...
function detail(m){
  ...
  return `<div class="match-detail">
    ...
    <section class="detail-card"><h3>PRICE CHECK</h3><div class="check"><span>Status</span><b class="${price.passed?'ok':'wait'}">${esc(priceLabel(m.priceStatus))}</b></div><div class="check"><span>Ah line</span><b>${fmtLine(line)}</b></div><div class="check"><span>Odds</span><b>${fmtOdds(odds)}</b></div><div class="check"><span>Source</span><b>${esc(market.source||'Odds-API.io')} · ${esc(market.bookmaker||'1xBet')}</b></div><div class="check"><span>Source updated</span><b>${market.sourceUpdatedAt?when(market.sourceUpdatedAt):'—'}</b></div><div class="check"><span>Price age</span><b>${price.ageSeconds!=null?`${Number(price.ageSeconds).toFixed(0)} sec`:'—'}</b></div></section>
  </div>`;
}
function matchRow(m){
  ...
  const hasPrice=market.status==='AH READY'&&n(line)!=null&&n(odds)!=null;
  const age=m.marketCheck?.ageSeconds;
  const ageText=age!=null?`${Number(age).toFixed(0)}s`:'—';
  const matchId=String(m.id??`${m.home}|${m.away}|${m.league}`);
  const currentPrice=hasPrice?`Ah line ${fmtLine(line)} · Odds ${fmtOdds(odds)} · ${esc(market.bookmaker||'1xBet')} · Age ${ageText}`:`Ah line — · Odds — · ${esc(market.bookmaker||'1xBet')} · Age —`;
  return `<details class="match-wrap ${st}" data-match-id="${esc(matchId)}" data-state="${esc(state)}" data-search="${esc(`${m.home} ${m.away} ${m.league}`.toLowerCase())}">
    ...
    <div class="market"><strong>${currentPrice}</strong><span class="${m.marketCheck?.passed?'ok':'wait'}">${esc(priceLabel(priceStatus))} · ${esc(market.source||'Odds-API.io')}</span></div>
...
async function healthPage(){
  ...
  const load=async()=>{try{const d=await get('/health');const c=d.counts||{},s=d.source||{},o=s.oddsApi||{};grid.innerHTML=`
    ...
    <article class="health-item"><header><b>Market feed</b><span class="${o.status==='READY'?'win':o.status==='ERROR'||o.status==='KEY_MISSING'?'loss':''}">● ${esc(o.status||'IDLE')}</span></header><div class="rows"><div><span>Source</span><strong>Odds-API.io</strong></div><div><span>Bookmaker</span><strong>1xBet</strong></div><div><span>Checked / mapped</span><strong>${o.checked??o.eligible??0} / ${o.mapped||0}</strong></div><div><span>AH ready</span><strong>${o.ready||c.market||0}</strong></div></div></article>
