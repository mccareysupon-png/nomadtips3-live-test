...
function setBars(stat,pair){
  const lines=stat.querySelectorAll('.event-stat-row');
  const values=[finite(pair?.home)?Number(pair.home):null,finite(pair?.away)?Number(pair.away):null];
  const max=Math.max(1,...values.filter(v=>v!==null));
  lines.forEach((line,index)=>{
    const bar=line.querySelector('.event-bar');
    if(!bar)return;
    const v=values[index];
    const width=v===null?0:Math.max(0,Math.min(100,(v/max)*100));
...
function clearOwned(card,label){
  const stat=statByLabel(card,label);if(!stat||stat.dataset.n342shotSource!=='5dollar')return;
  const lines=stat.querySelectorAll('.event-stat-row');
  lines[0]?.querySelector('b')?.replaceChildren('—');
  lines[1]?.querySelector('b')?.replaceChildren('—');
  lines.forEach(line=>{const bar=line.querySelector('.event-bar');if(bar)bar.style.width='0%'});
  const small=stat.querySelector('small');if(small){small.textContent='ROLLING CHANGE · HOME — · AWAY —';small.removeAttribute('title')}
  delete stat.dataset.n342shotSource;
  delete stat.dataset.n342shotDisplayOnly;
}
function applyCard(card){
  const id=String(card.dataset.matchId||'');if(!id)return;
  const row=rowsByMatch.get(id);
  if(!row||row.status!=='MATCHED'){
    clearOwned(card,'SHOTS ON TARGET');
    clearOwned(card,'SHOTS OFF TARGET');
    return;
  }
