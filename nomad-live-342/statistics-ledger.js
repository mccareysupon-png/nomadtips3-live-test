...
function ensureHistoryStyle(){
  ...
  const style=document.createElement('style');style.id='nomad342-history-style';style.textContent=`
    ...
    #nomad342PanelStatistics .stats-history-day-dot{fill:#68e5aa;stroke:#102a20;stroke-width:1.5;vector-effect:non-scaling-stroke;opacity:.72}
    ...
    #nomad342PanelStatistics .stats-history-daily{position:relative;display:flex;gap:5px;margin:10px -2px 0;padding:2px 2px 6px;overflow-x:auto;overscroll-behavior-inline:contain;scrollbar-width:thin;scrollbar-color:rgba(65,217,154,.18) transparent}
    #nomad342PanelStatistics .stats-history-day{flex:0 0 auto;min-width:76px;padding:7px 8px 6px;border:0;background:rgba(255,255,255,.025);color:#7d8a82;text-align:left;cursor:pointer;font-family:Arial,Helvetica,sans-serif}
    #nomad342PanelStatistics .stats-history-day:hover,#nomad342PanelStatistics .stats-history-day.is-active{background:rgba(65,217,154,.075);box-shadow:inset 0 -1px 0 rgba(65,217,154,.35)}
    #nomad342PanelStatistics .stats-history-day-date{display:block;color:#a7b3ab;font:900 8px/1 Arial,Helvetica,sans-serif;letter-spacing:.07em}
    #nomad342PanelStatistics .stats-history-day-result{display:block;margin-top:5px;color:#87948b;font:900 9px/1 Arial,Helvetica,sans-serif;white-space:nowrap}
    #nomad342PanelStatistics .stats-history-day-rate{display:block;margin-top:4px;color:#58dda0;font:900 10px/1 Arial,Helvetica,sans-serif}
    @media(max-width:700px){
      ...
      #nomad342PanelStatistics .stats-history-daily{margin-top:8px;padding-bottom:5px}
      #nomad342PanelStatistics .stats-history-day{min-width:72px;padding:6px 7px}
    }
...
function renderHistory(nextDaily){
  ...
  const rows=historyRows(),plot=card.querySelector('#statsHistoryPlot'),dailyNode=card.querySelector('#statsHistoryDaily'),rateNode=card.querySelector('#statsHistoryRate'),metaNode=card.querySelector('#statsHistoryMeta'),fromNode=card.querySelector('#statsHistoryFrom'),toNode=card.querySelector('#statsHistoryTo');
  if(!rows.length){rateNode.innerHTML='—<small>WIN RATE</small>';metaNode.textContent='NO HISTORY';fromNode.textContent='—';toNode.textContent='—';plot.innerHTML='<div class="stats-history-empty">Waiting for settled history…</div>';if(dailyNode)dailyNode.innerHTML='';return;}
  ...
  const dayRadius=rows.length<=31?3.2:rows.length<=90?2.5:1.8;
  const dayDots=points.map(point=>`<circle class="stats-history-day-dot" cx="${point.x.toFixed(2)}" cy="${point.y.toFixed(2)}" r="${dayRadius}"/>`).join('');
  plot.innerHTML=`<svg class="stats-history-svg" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="statsHistoryFill342" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#41d99a" stop-opacity=".28"/><stop offset="62%" stop-color="#41d99a" stop-opacity=".07"/><stop offset="100%" stop-color="#41d99a" stop-opacity="0"/></linearGradient><filter id="statsHistoryGlow342" x="-20%" y="-30%" width="140%" height="180%"><feGaussianBlur stdDeviation="6"/></filter></defs>${grids}<path d="${area}" fill="url(#statsHistoryFill342)"/><path d="${line}" fill="none" stroke="#41d99a" stroke-opacity=".34" stroke-width="7" filter="url(#statsHistoryGlow342)" vector-effect="non-scaling-stroke"/><path d="${line}" fill="none" stroke="#59e2a5" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>${dayDots}<circle id="statsHistoryDot" cx="${points[points.length-1].x}" cy="${points[points.length-1].y}" r="4.6" fill="#8af3c0" stroke="#173e2d" stroke-width="2" vector-effect="non-scaling-stroke" opacity="0"/></svg><div class="stats-history-tooltip" id="statsHistoryTooltip"></div><div class="stats-history-hitlayer" id="statsHistoryHitlayer" aria-label="Hover or tap the chart for daily details"></div>`;
  if(dailyNode)dailyNode.innerHTML=rows.map((row,index)=>{const rate=finite(row?.winRate)||0;return `<button type="button" class="stats-history-day" data-history-day="${index}" title="${esc(historyDate(row?.day,true))}"><span class="stats-history-day-date">${esc(historyDate(row?.day).toUpperCase())}</span><span class="stats-history-day-result">W ${esc(row?.wins??0)} · L ${esc(row?.losses??0)} · D ${esc(row?.pushes??0)}</span><span class="stats-history-day-rate">${rate.toFixed(1)}%</span></button>`;}).join('');
  ...
  const show=index=>{index=Math.max(0,Math.min(points.length-1,index));const point=points[index],pct=point.x/W*100;dot.setAttribute('cx',point.x);dot.setAttribute('cy',point.y);dot.setAttribute('opacity','1');tooltip.innerHTML=historyTooltip(point.row);tooltip.style.left=`${pct}%`;tooltip.style.top=`${point.y/H*100}%`;tooltip.classList.toggle('is-left',pct<16);tooltip.classList.toggle('is-right',pct>84);tooltip.classList.add('is-visible');const next=dailyNode?.querySelector(`[data-history-day="${index}"]`)||null;if(activeDaily!==next){activeDaily?.classList.remove('is-active');activeDaily=next;activeDaily?.classList.add('is-active');}};
  ...
  hit.addEventListener('pointermove',locate);hit.addEventListener('pointerdown',locate);hit.addEventListener('pointerleave',()=>{dot.setAttribute('opacity','0');tooltip.classList.remove('is-visible');activeDaily?.classList.remove('is-active');activeDaily=null;});
  if(dailyNode){const dailyLocate=event=>{const item=event.target.closest('[data-history-day]');if(item)show(Number(item.dataset.historyDay)||0);};dailyNode.onpointerover=dailyLocate;dailyNode.onclick=dailyLocate;dailyNode.onpointerleave=()=>{dot.setAttribute('opacity','0');tooltip.classList.remove('is-visible');activeDaily?.classList.remove('is-active');activeDaily=null;};}
}
