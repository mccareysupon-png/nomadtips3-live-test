...
function matchCard(r){
  const m=r.m,em=r.event.metrics,id=String(m.id),expanded=expandedMatches.has(id),stale=Boolean(m.freshness?.stale),state=stale?'STALE':r.event.pass?'EVENT':'WATCH';
