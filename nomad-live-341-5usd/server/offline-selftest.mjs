...
for(let i=0;i<45;i++){
  const observedAt=new Date(start+i*15000).toISOString();
...
console.log(JSON.stringify({ok:true,cycles:state.cycleNumber,providerRequests:snapshot.providerRequestCount,matches:snapshot.matches.length,signals:snapshot.counts.signal,ledger:snapshot.ledger.length,rollingReady:snapshot.matches.filter(m=>m.rolling?.available).length},null,2));
