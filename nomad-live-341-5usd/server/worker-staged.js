...
const CONTROL_DEFAULTS={masterApiEnabled:false,pollingEnabled:false,activeMachineId:'341-STAGED-A',pollIntervalSeconds:15,requestTimeoutMs:15000,revision:0,updatedAt:null,updatedBy:'SYSTEM',leaseOwner:null,leaseId:null,leaseUntilMs:0,lastRunAt:null,lastRunResult:'NEVER'};
...
const normalizeControl=input=>({...CONTROL_DEFAULTS,...(input&&typeof input==='object'?input:{}),masterApiEnabled:Boolean(input?.masterApiEnabled),pollingEnabled:Boolean(input?.pollingEnabled),activeMachineId:normalizeMachine(input?.activeMachineId||CONTROL_DEFAULTS.activeMachineId),pollIntervalSeconds:clamp(input?.pollIntervalSeconds??15,6,300),requestTimeoutMs:clamp(input?.requestTimeoutMs??15000,3000,60000),revision:Number(input?.revision)||0,leaseUntilMs:Number(input?.leaseUntilMs)||0});
  ...
  async fetch(request){
    ...
    if(url.pathname==='/lease/acquire'){
      ...
      await this.ctx.storage.transaction(async txn=>{
        const current=normalizeControl(await txn.get('control'));
        const now=Date.now();
        if(current.leaseUntilMs>now&&current.leaseId){result={acquired:false,control:current,reason:'LEASE_BUSY'};return;}
        ...
        await txn.put('control',next);result={acquired:true,leaseId,control:next};
      });
...
async function loadState(env){
  if(env?.NOMAD341_STATE?.get){const raw=await env.NOMAD341_STATE.get('runtime-state');if(raw){try{return createRuntimeState(JSON.parse(raw));}catch{}}}
  return createRuntimeState(memoryState);
}
async function saveState(env,state){memoryState=createRuntimeState(state);if(env?.NOMAD341_STATE?.put)await env.NOMAD341_STATE.put('runtime-state',JSON.stringify(memoryState));}

function permission(env,control){const current=machineId(env),machineAllowed=normalizeMachine(control.activeMachineId)===current;return {hardProviderGate:hardLiveEnabled(env),currentMachineId:current,machineAllowed,effectiveProviderEnabled:hardLiveEnabled(env)&&control.masterApiEnabled&&machineAllowed};}
function publicControl(env,control){const p=permission(env,control);return {...control,...p,persistence:controlStub(env)?'DURABLE_OBJECT':'MEMORY_PREVIEW'};}
...
export async function runCentralCycle(env={},options={}){
  ...
  cycleInFlight=(async()=>{
    ...
    try{
      ...
      await saveState(env,state);await patchControl(env,{lastRunAt:startedAt,lastRunResult:`ERROR:${String(error?.message||error).slice(0,80)}`,updatedBy:'RUNTIME'});
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==='/api/nomad341/control'&&request.method==='POST'){
      ...
      const patch={updatedBy:'OWNER'};for(const k of ['masterApiEnabled','pollingEnabled','activeMachineId','pollIntervalSeconds','requestTimeoutMs'])if(k in body)patch[k]=body[k];
