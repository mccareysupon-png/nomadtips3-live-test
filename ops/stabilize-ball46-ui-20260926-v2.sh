...
# Shared desktop chrome lives INSIDE workspace-stage so the outer workspace grid never changes.
p=Path('nomad-live-343/singlepage-workspace-343.js')
s=p.read_text()
if 'BALL46_UI_STABLE_CHROME_20260926' in s: raise SystemExit('STABLE_CHROME_ALREADY_PRESENT')
...
# Stable 10-cell scorebar uses fixtures already held by the board renderer; no request is added.
p=Path('nomad-live-343/dashboard-v2-stage3.js')
s=p.read_text()
if 'BALL46_WORKSPACE_SCOREBAR_20260926' in s: raise SystemExit('WORKSPACE_SCOREBAR_ALREADY_PRESENT')
...
# Fixed shared desktop geometry. Mobile keeps its existing presentation.
...
# Race guard: abort if any target moved after the mirror.
for f in dashboard-v2-stage3.js singlepage-workspace-343.js singlepage-workspace-343.css; do
