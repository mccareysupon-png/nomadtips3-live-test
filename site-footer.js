(()=>{
  ...
  const normalizePublicMarketNav = () => {
    ...
    const links = document.querySelectorAll(
      '.topbar-inner.public-four-nav > .topnav a, .mobile-nav.public-four-nav a'
    );
    let found = false;
    links.forEach(link => {
      ...
      try {
        pointsTo342 = /\/nomad-live-342(?:\/|$)/i.test(new URL(link.getAttribute('href') || '', window.location.href).pathname);
      } catch (_) {}
