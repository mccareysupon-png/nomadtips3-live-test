...
export async function handleV2Route(request, env) {
  ...
  if (url.pathname === '/v2/ingest') {
    ...
    try {
      ...
      if (!validEnvelope(envelope)) {
        return reply(request, { ok: false, error: 'INVALID_ENVELOPE' }, 400);
      }
  ...
  if (url.pathname === '/v2/state') {
    ...
    if (!latest) return reply(request, { ok: true, state: null }, 200);
    return reply(request, { ok: true, state: publicPayload(latest) });
  }

  if (url.pathname === '/v2/owner/config') {
    if (!authorized(request, env.V2_OWNER_SECRET)) {
      return reply(request, { ok: false, error: 'UNAUTHORIZED' }, 401);
    }
    ...
    if (request.method === 'PUT') {
      try {
        ...
        const result = await writeOwnerConfig(
          env,
          body.config || {},
        ...
        if (result.conflict) {
          return reply(request, { ok: false, error: 'VERSION_CONFLICT', current: result.current }, 409);
        }
