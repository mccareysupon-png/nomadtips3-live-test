...
(async()=>{
  ...
  for(const viewport of [{width:1366,height:900,name:'desktop'},{width:390,height:844,name:'mobile'}]){
    ...
    const state=card.locator('[data-field="signal-text"]');
    const summary=card.locator('[data-field="signal-summary"]');
    ...
    if(before.state!=='SIGNAL LOCKED')throw Error(viewport.name+' SIGNAL_STATE_BAD '+JSON.stringify(before));
