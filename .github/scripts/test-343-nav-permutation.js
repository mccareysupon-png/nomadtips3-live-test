...
const close = (a,b,tol=0.75) => a !== null && b !== null && Math.abs(a-b) <= tol;
  ...
  for (const vp of VIEWPORTS) {
    ...
    for (const order of ORDERS) {
      ...
      const snap = async(label) => page.evaluate((label)=>{
        ...
        const nav = [...document.querySelectorAll('.topnav,.mobile-nav')].find(el => getComputedStyle(el).display !== 'none')?.getBoundingClientRect();
        const critical = document.querySelector('style[data-nomad-nav-stability]')?.getAttribute('data-nomad-nav-stability') || '';
        const appHref = document.querySelector('link[href^="app.css"]')?.getAttribute('href') || '';
        return {
          label,
          ...
          navLeft: nav?.left ?? null,
          navTop: nav?.top ?? null,
          navWidth: nav?.width ?? null,
          navHeight: nav?.height ?? null,
          desktopNavCount: document.querySelectorAll('.topnav').length,
      ...
      const settleAndCheck = async(label) => {
        ...
        for (const [x,y,name] of [
          ...
          if (!close(x,y)) throw new Error(`${vp.name} ${order.join('>')} ${label} LATE_SHAKE ${name} ${x} -> ${y}`);
        }
      ...
      for (const s of states) {
        if (!close(s.topLeft,first.topLeft) || !close(s.topWidth,first.topWidth) || !close(s.topHeight,first.topHeight)) {
          throw new Error(`${vp.name} ${order.join('>')} TOPBAR_SHAKE ${JSON.stringify(states)}`);
        }
        if (!close(s.shellLeft,first.shellLeft) || !close(s.shellTop,first.shellTop) || !close(s.shellWidth,first.shellWidth)) {
          throw new Error(`${vp.name} ${order.join('>')} SHELL_SHAKE ${JSON.stringify(states)}`);
        }
        if (!close(s.navLeft,first.navLeft) || !close(s.navTop,first.navTop) || !close(s.navWidth,first.navWidth) || !close(s.navHeight,first.navHeight)) {
          throw new Error(`${vp.name} ${order.join('>')} NAV_SHAKE ${JSON.stringify(states)}`);
        }
      }

      const key = order[0];
      if (!baselineByPage.has(key)) baselineByPage.set(key, first);
