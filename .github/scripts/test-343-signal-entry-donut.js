...
(async()=>{
  ...
  const labels=result.labels.map(x=>String(x||'').toLowerCase().replace(/\s+/g,' ').trim()).join('|');
  if(labels!=='attacks|dangerous attacks|shots on target|shots off target|corners|possession')throw Error('LABELS_BAD '+JSON.stringify(result));
