...
function renderMatch(){const m=matchById(selectedId);if(!m){$('#match').innerHTML='<div class="empty">Select a live match.</div>';$('#stats').innerHTML='';$('#events').innerHTML='';return;}const r=selectedSignal?recordById(selectedId):null,sel=r?.selectedSide;$('#match').innerHTML=`<div class="match-card"><div class="team home-team ${sel==='HOME'?'selected':''}"><small>HOME</small><b>${esc(m.home)}</b></div><div class="score"><b>${esc(m.score.home)}–${esc(m.score.away)}</b><span data-clock="${esc(m.id)}">${esc(clockText(m.id))}</span></div><div class="team away-team ${sel==='AWAY'?'selected':''}"><small>AWAY</small><b>${esc(m.away)}</b></div></div>${renderPitch(m)}${renderChart(m)}`;$('#stats').innerHTML=renderStats(m);$('#events').innerHTML=renderEvents(m);document.querySelectorAll('[data-metric]').forEach(btn=>btn.onclick=()=>{activeMetric=btn.dataset.metric;renderMatch();updateClocks();applyPitchState(m.id);});applyPitchState(m.id);}
