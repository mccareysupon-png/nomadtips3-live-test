...
export const EVENT_TYPES={
  20:'DANGEROUS ATTACK',21:'ATTACK',22:'POSSESSION',23:'GOAL',
  24:'YELLOW CARD',25:'RED CARD',26:'DANGEROUS FREE KICK',27:'PENALTY',
  28:'SHOT ON TARGET',29:'SHOT OFF TARGET',30:'SUBSTITUTION',31:'OFFSIDE',
  32:'FREE KICK',33:'THROW IN',34:'CORNER',35:'PENALTY GOAL',
  36:'PENALTY MISSED',37:'FOUL',38:'GOAL DISALLOWED',39:'DANGEROUS FREE KICK',
  40:'STOPPAGE',41:'SHOT BLOCKED',42:'INJURY TIME',43:'VAR CHECKING',
  44:'VAR RED CARD',45:'VAR GOAL DISALLOWED',46:'VAR PENALTY'
};
...
function graphRow(raw,meta,pointsByEvent){
  ...
  return{
    id,teamId,team,code,type:EVENT_TYPES[code]||`EVENT ${code}`,
    location:num(p[3]),state:num(p[4]),minute:num(p[5]),injuryMinute:num(p[6])||0,
    eventId,playerNumber:String(p[10]||''),playerName:String(p[11]||''),
...
export function parseFlashData(source,{matchId,homeTeamId=null,awayTeamId=null}={}){
  ...
  for(const point of points){
    if(!pointsByEvent.has(point.eventId))pointsByEvent.set(point.eventId,[]);
    pointsByEvent.get(point.eventId).push(point);
  }
...
async function fetchFlash(matchId,mode){
  ...
  const response=await fetch(url,{
    headers:{
      'user-agent':'NOMADTIPS3-CAR3.3-Animation/1.0 (+public live activity)',
      'accept':'text/plain,*/*','accept-language':'en-US,en;q=.8'
    },
    cf:{cacheTtl:full?30:2,cacheEverything:true}
  });
  if(!response.ok)throw new Error(`FLASHDATA_${mode.toUpperCase()}_HTTP_${response.status}`);
  return response.text();
}

export async function handleAnimationRequest(request,env){
  ...
  const objectId=env.CAR33_STATE.idFromName('engine'),stub=env.CAR33_STATE.get(objectId);
  const liveResp=await stub.fetch('https://car33.internal/live');
  const livePayload=await liveResp.json().catch(()=>null);
  const match=(livePayload?.matches||[]).find(m=>String(m.id||m.sourceMatchId)===matchId);
  if(!match)return json({ok:false,matchId,reason:'MATCH_NOT_IN_CURRENT_CAR33_LIVE_FEED'},404);
  ...
  if(!fullResult.ok)return json({ok:false,matchId,reason:'ANIMATION_SOURCE_UNAVAILABLE',sourceError:fullResult.error,fallback:'HOLD_LAST_SOURCE_POSITION'},200,'public, max-age=1');
  const full=parseFlashData(fullResult.value,{matchId});
  if(!full.ok||!full.homeTeamId||!full.awayTeamId)return json({ok:false,matchId,reason:'ANIMATION_SOURCE_PARSE_INCOMPLETE',fallback:'HOLD_LAST_SOURCE_POSITION'},200,'public, max-age=1');
  ...
  const frame=mergeAnimationFrames(full,change?.ok?change:null);
  return json({
    ok:true,matchId,generatedAt:new Date().toISOString(),pollMs:POLL_MS,
    source:'NOMADTIPS3 · PUBLIC LIVE ACTIVITY',
    sourceTransport:'GOALOO_PUBLIC_FLASHDATA_HTTP',
    sourceCoordinateSystem:'NORMALIZED_0_1',
    sourceMode:change?.ok?'CHANGE+FULL_METADATA':'FULL_ONLY',
    ...
    events:frame.events,current:frame.current,
    fallback:'HOLD_LAST_SOURCE_POSITION'
