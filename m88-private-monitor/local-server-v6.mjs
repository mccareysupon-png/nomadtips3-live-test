...
function mergeRow(old,r){
  ...
  out.markets=uniq([...(old.markets||[]),...(r.markets||[])],x=>[x.market,x.selection,x.line??'',x.odds??''].join('|'),1200);
  ...
  if(pref){out.market=pref.market;out.selection=pref.selection;out.odds=pref.odds;out.line=pref.line;out.handicapLine=pref.handicapLine;out.goalLine=pref.goalLine;}
  return out;
}
...
function footballEvidence(r){
  ...
  if(/1x2|match result|asian handicap|over.?under|total goals/.test(mk))score+=20;
...
function aggregate(){
  ...
  for(const r of state.rows.values()){
    ...
    const key=realEventId(r.sourceId)?String(r.sourceId):`${h}|${a}`;
  ...
  for(const [eventKey,rows] of groups){
    ...
    merged.realSourceContract=merged.sourceVerified?'NETWORK_OR_WEBSOCKET_EVENT_ID':'UNVERIFIED';
    merged.rawRowCount=rows.length;out.push(merged);
  }
...
let timer=null;function persistSoon(){
  clearTimeout(timer);timer=setTimeout(()=>{
    ...
    fs.writeFileSync(path.join(DATA,'latest-real.json'),JSON.stringify({updatedAt:new Date().toISOString(),contract:'M88-REAL-SOURCE-v1',matchCount:matches.length,matches},null,2));
    fs.writeFileSync(path.join(DATA,'diagnostics-real.json'),JSON.stringify({updatedAt:new Date().toISOString(),note:'DOM is diagnostic only and never enters the decision feed',rowCount:state.rows.size,frames:state.frames,sources:[...state.sources.values()].sort((a,b)=>b.rows-a.rows||b.hits-a.hits),domSamples:state.domSamples,rawSamples:[...state.rawSamples.values()].slice(-30)},null,2));
  },350);
}
...
const server=http.createServer((req,res)=>{
  ...
  if(u.pathname==='/api/health')return send(res,{ok:true,service:'m88-pc-collector-v6-real-source-only',contract:'M88-REAL-SOURCE-v1',browser:state.browser,pageUrl:state.pageUrl,lastNetworkAt:state.lastNetworkAt,lastUsefulAt:state.lastUsefulAt,lastDomAt:state.lastDomAt,domRole:'diagnostic-only',footballFocused:state.footballFocused,footballFocusedAt:state.footballFocusedAt,sourceCount:state.sources.size,frameCount:state.frames.length,rowCount:state.rows.size,allNetworkMatchCount:matches.length,detectionMatchCount:detection.length,verifiedSourceMatches:matches.filter(x=>x.sourceVerified).length,richStatsMatches:detection.filter(x=>(x.statsCoverage||0)>0).length,eventMatches:detection.filter(x=>(x.eventCount||0)>0).length,marketMatches:detection.filter(x=>(x.marketCount||0)>0).length,startedAt:state.startedAt});
  if(u.pathname==='/api/feed')return send(res,{ok:true,mode:'m88-real-network-ws-only-v6',contract:'M88-REAL-SOURCE-v1',checkedAt:new Date().toISOString(),allNetworkMatchCount:matches.length,matchCount:detection.length,message:detection.length?'Verified M88 live football events from Network/WebSocket only.':'No verified M88 Network/WebSocket live football event mapped. DOM/UI data is intentionally ignored.',matches:detection});
  if(u.pathname==='/api/event-feed')return send(res,{ok:true,mode:'m88-network-ws-events-v6-all',contract:'M88-REAL-SOURCE-v1',checkedAt:new Date().toISOString(),matchCount:matches.length,matches});
  if(u.pathname==='/api/source/probe')return send(res,{ok:true,mode:'m88-real-source-probe-v6',contract:'M88-REAL-SOURCE-v1',domRole:'diagnostic-only',pageUrl:state.pageUrl,frames:state.frames,sources:[...state.sources.values()].sort((a,b)=>b.rows-a.rows||b.hits-a.hits).slice(0,150),matchPreview:matches.slice(0,40).map(x=>({eventId:x.sourceId,sourceKind:x.sourceKind,sourceVerified:x.sourceVerified,home:x.home,away:x.away,sport:x.sport,league:x.league,status:x.status,minute:x.minute,isLive:x.isLive,isVirtual:x.isVirtual,isFootball:x.isFootball,footballConfidence:x.footballConfidence,statsCoverage:x.statsCoverage,marketCount:x.marketCount,eventCount:x.eventCount,odds:x.odds,market:x.market,line:x.line})),domSamples:state.domSamples,rawSamples:[...state.rawSamples.values()].slice(-10)});
  return staticFile(req,res);
});
server.listen(PORT,'127.0.0.1',()=>console.log(`M88 private monitor v6 REAL source only: http://127.0.0.1:${PORT}`));
...
async function launch(){
  ...
  console.log('Collector v6 running: Network/WebSocket are decision sources. DOM/UI is diagnostic only.');
}
