...
// Viewer actions read odds already carried by the Ball46 bulk snapshot first.
// Central cache is fallback-only and never triggers a 5USD provider request.
const VERSION='343-live-summary-viewer-readonly-bulk-provider-v2';
...
function renderPinned(expanded,fixture,owner='embedded-full-odds-read-only'){if(!expanded?.isConnected||!fixture)return;defer(()=>{if(!expanded?.isConnected)return;const renderer=window.NOMAD343_FULL_MARKET_BOOKMAKER;if(!renderer?.update)return;expanded._nomadRichFixture=fixture;expanded.dataset.oddsRenderOwner=owner;renderer.update(expanded,fixture)})}
function paint(expanded,fixture,hit){if(expanded?.isConnected&&fixture&&hit?.fullOdds)renderPinned(expanded,richFixture(fixture,hit),'rich-odds-cache-only')}
...
function onFixtureReady(e){
  const fixture=e?.detail?.fixture,id=idOf(fixture),expanded=e.target?.closest?.('.match-expanded')||document.querySelector(`.match-expanded[data-expanded-match="${CSS.escape(id)}"]`);
  if(!id||!expanded)return;
  // The paid Bulk snapshot already carries the bookmaker pack in providerOdds.
  // Render it directly; do not wait for or request per-fixture Full Odds.
  if(hasData(fixture?.providerOdds)){renderPinned(expanded,fixture,'bulk-provider-odds-read-only');return}
  if(hasData(fixture?.fullOdds)){renderPinned(expanded,fixture,'embedded-full-odds-read-only');return}
  const hit=local(id);if(hit){paint(expanded,fixture,hit);return}
  readCentral([id]).then(rows=>paint(expanded,fixture,rows.get(id))).catch(err=>console.warn('Central odds cache read unavailable',err))
}
function start(){document.addEventListener('nomad343:fixture-ready',onFixtureReady);document.addEventListener('nomad343:signal-filter-active',onSignalFilter);window.NOMAD343_LIVE_SUMMARY_FULL_ODDS={version:VERSION,mode:'VIEWER_READ_ONLY',networkMode:'BULK_PROVIDER_FIRST_CACHE_FALLBACK',renderOwner:'BULK_PROVIDER_ODDS_READ_ONLY',automaticPolling:false,defaultCardVisibleEnrichment:false,signalFilterPrefetch:false,signalCacheRead:true,viewerTriggeredProviderFetch:false,fanout:false,clientCacheMs:CLIENT_CACHE_MS,source:'ENGINE_BOARD_BULK_PROVIDER_ODDS_PLUS_CACHE_FALLBACK',upstreamRequestsPerViewer:0,current:id=>cache.get(String(id))||null,clear:()=>cache.clear()}}
