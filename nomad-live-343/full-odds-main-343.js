(()=>{
'use strict';
const VERSION='343-full-odds-main-v6-shared-batch';
const BOOKS=[
...
function render(card,payload){
  ...
  host.innerHTML=`<section class="fom-board"><div class="fom-head"><div><b>FULL MARKET · 10 BOOKS</b><small>Fixed sides: home left · away right · shared batch feed · zero per-match API calls</small></div><span class="fom-count${payload?.stale?' muted':''}">${esc(count)}</span></div><div class="fom-team-strip"><div class="home"><small>HOME · LEFT</small><strong title="${esc(home)}">${esc(home)}</strong></div><i>VS</i><div class="away"><small>AWAY · RIGHT</small><strong title="${esc(away)}">${esc(away)}</strong></div></div><div class="fom-grid">${rows.join('')}</div></section>`;
}
function batchSnapshot(){return window.NOMAD343_BATCH_STATE?.snapshot??window.NOMAD343_BET365?.snapshot??null}
