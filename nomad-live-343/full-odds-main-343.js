...
const STAGES=[['inplay','IN-PLAY'],['in_play','IN-PLAY'],['live','IN-PLAY'],['closing','PRE-MATCH'],['opening','OPENING']];
...
function oddsRoot(raw){
  ...
  if(root.odds&&typeof root.odds==='object'&&!('opening' in root)&&!('closing' in root)&&!('inplay' in root)&&!('in_play' in root)&&!('live' in root))return root.odds;
  return root;
}
...
function stageHtml(key,label,v,card){
  ...
  if(k.startsWith('1x2')){
    const h=num(v.home),d=num(v.draw),a=num(v.away),rows=[];
    if(h!==null)rows.push(chip(home,fmt(h)));
    if(d!==null)rows.push(chip('DRAW',fmt(d)));
    if(a!==null)rows.push(chip(away,fmt(a)));
  ...
  if(asian){
    ...
    const rows=[];
    if(h!==null)rows.push(chip(home,`${line===null?'':signed(line)+' @ '}${fmt(h)}`));
    if(a!==null)rows.push(chip(away,`${line===null?'':signed(-line)+' @ '}${fmt(a)}`));
  ...
  if(total){
    ...
    const rows=[];
    if(over!==null)rows.push(chip('OVER',`${line===null?'':fmt(line)+' @ '}${fmt(over)}`));
    if(under!==null)rows.push(chip('UNDER',`${line===null?'':fmt(line)+' @ '}${fmt(under)}`));
    return `<div class="fom-stage"><b>${label}</b><div class="fom-chips">${rows.join('')}</div></div>`;
  }
  if(k==='btts'){
    const yes=num(v.yes),no=num(v.no),rows=[];
    if(yes!==null)rows.push(chip('YES',fmt(yes)));
    if(no!==null)rows.push(chip('NO',fmt(no)));
    return rows.length?`<div class="fom-stage"><b>${label}</b><div class="fom-chips">${rows.join('')}</div></div>`:'';
  }
  return'';
}
function marketHtml(key,value,card){
  ...
  for(const [stage,label] of STAGES){
    if(seen.has(label)||value[stage]===undefined||value[stage]===null)continue;
    const html=stageHtml(key,label,value[stage],card);
    if(html){seen.add(label);stages.push(html)}
  }
...
function render(card,payload){
  ...
  if(!rows.length){host.innerHTML=`<section class="fom-board"><div class="fom-head"><div><b>BET365 · FULL ODDS</b><small>${esc(home)} vs ${esc(away)} · 5USD FULL ODDS</small></div><span class="fom-count muted">ODDS —</span></div><div class="fom-empty">Full prices unavailable for this fixture</div></section>`;return}
  host.innerHTML=`<section class="fom-board"><div class="fom-head"><div><b>BET365 · FULL ODDS</b><small>${esc(home)} vs ${esc(away)} · 5USD FULL ODDS</small></div><span class="fom-count">${rows.length} MARKETS</span></div><div class="fom-grid">${rows.join('')}</div></section>`;
}
function loading(card){const host=ensureHost(card);if(host)host.innerHTML='<section class="fom-board"><div class="fom-head"><div><b>BET365 · FULL ODDS</b><small>กำลังโหลด Line + Odds เต็มจาก Bet365</small></div><span class="fom-count muted">LOADING</span></div></section>'}
async function load(card){
  ...
  }catch(e){const host=ensureHost(card);if(host)host.innerHTML=`<section class="fom-board"><div class="fom-head"><div><b>BET365 · FULL ODDS</b><small>${esc(String(e?.message||e))}</small></div><span class="fom-count muted">ODDS —</span></div><div class="fom-empty">ไม่มีราคาน้ำเต็มจากต้นทางในรอบนี้</div></section>`}
