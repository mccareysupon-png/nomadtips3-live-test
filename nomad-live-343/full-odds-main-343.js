(()=>{
'use strict';
const VERSION='343-full-odds-main-v2-single-box';
const BOARD_API='/api/engine/board';
const FULL_API='/api/engine/fixture-odds';
const POLL_MS=30_000;
const FULL_CACHE_MS=60_000;
let lastSnapshot=null,boardBusy=false;
const fullCache=new Map(),fullBusy=new Set();
...
const STAGES=[['inplay','IN-PLAY'],['in_play','IN-PLAY'],['current','CURRENT'],['closing','PRE-MATCH'],['opening','OPENING']];
...
function rootFrom(raw){
  const root=raw?.fullOdds??raw?.providerOdds??raw?.odds??raw?.data??raw;if(!root||typeof root!=='object')return null;
  ...
  if(root.bookmakers&&typeof root.bookmakers==='object'&&!Array.isArray(root.bookmakers)){const x=rootFrom(root.bookmakers);if(x)return x}
...
function stageHtml(key,label,v,card){
  ...
  if(asian){
    if(typeof v!=='object')return `<div class="fom-stage"><b>${label}</b><div class="fom-chips">${chip('HOME LINE',signed(v))}</div></div>`;
  ...
  if(total){
    if(typeof v!=='object')return `<div class="fom-stage"><b>${label}</b><div class="fom-chips">${chip('LINE',fmt(v))}</div></div>`;
...
function marketHtml(key,value,card){
  ...
  const obj=(value&&typeof value==='object')?value:{current:value},seen=new Set(),stages=[];
  for(const [stage,label] of STAGES){if(seen.has(label)||obj[stage]===undefined||obj[stage]===null)continue;const html=stageHtml(key,label,obj[stage],card);if(html){seen.add(label);stages.push(html)}}
  if(!stages.length){const html=stageHtml(key,'CURRENT',obj,card);if(html)stages.push(html)}
...
function ensureHost(card){
  ...
  if(flow)flow.insertAdjacentElement('afterend',host);else if(stats)stats.insertAdjacentElement('afterend',host);else details.append(host);
  return host;
}
function panel(card,root,source){
  const entries=root&&typeof root==='object'?Object.entries(root).filter(([k,v])=>!['fixture_id','bookmaker','slug','name','updated_at','recorded_at'].includes(k)&&v!==undefined&&v!==null).sort((a,b)=>rank(a[0])-rank(b[0])):[];
  const rows=entries.map(([k,v])=>marketHtml(k,v,card)).filter(Boolean),home=team(card,'home'),away=team(card,'away');
  if(!rows.length)return `<section class="fom-board"><div class="fom-head"><div><b>BET365 · LIVE MARKETS</b><small>${esc(home)} vs ${esc(away)} · ${esc(source)}</small></div><span class="fom-count muted">ODDS —</span></div><div class="fom-empty">Odds unavailable for this fixture</div></section>`;
  return `<section class="fom-board"><div class="fom-head"><div><b>BET365 · LIVE MARKETS</b><small>${esc(home)} vs ${esc(away)} · ${esc(source)}</small></div><span class="fom-count">${rows.length} MARKETS</span></div><div class="fom-grid">${rows.join('')}</div></section>`;
}
function fixtureMap(){return new Map((Array.isArray(lastSnapshot?.fixtures)?lastSnapshot.fixtures:[]).map(f=>[String(f?.fixtureId??''),f]))}
function renderCard(card){
  const host=ensureHost(card);if(!host)return;
  const id=String(card.dataset.matchId||''),f=fixtureMap().get(id),full=fullCache.get(id);
  if(full?.data){host.innerHTML=panel(card,rootFrom(full.data),'5USD FULL ODDS');return}
  if(f){host.innerHTML=panel(card,rootFrom(f?.providerOdds??f?.odds),'5USD BATCH · กดคู่เพื่อโหลด FULL ODDS');return}
  host.innerHTML=panel(card,null,'WAITING FOR BOARD');
}
function decorate(){document.querySelectorAll('.match-card[data-match-id]').forEach(card=>{renderCard(card);if(card.getAttribute('aria-expanded')==='true')loadFull(card)})}
async function loadBoard(){
  if(boardBusy)return;boardBusy=true;
  try{const r=await fetch(`${BOARD_API}?_=${Date.now()}`,{cache:'no-store'}),j=await r.json().catch(()=>null);if(r.ok&&j?.ok===true){lastSnapshot=j;decorate()}}
  catch(e){console.warn('[NOMAD343 ODDS BOARD]',e)}finally{boardBusy=false}
}
async function loadFull(card){
  const id=String(card?.dataset?.matchId||'');if(!id||fullBusy.has(id))return;
  const hit=fullCache.get(id);if(hit&&Date.now()-hit.at<FULL_CACHE_MS){renderCard(card);return}
  fullBusy.add(id);
  const host=ensureHost(card);if(host&&!hit){const f=fixtureMap().get(id);host.innerHTML=panel(card,rootFrom(f?.providerOdds??f?.odds),'กำลังโหลด FULL ODDS…')}
  try{
    const r=await fetch(`${FULL_API}?fixtureId=${encodeURIComponent(id)}&_=${Date.now()}`,{cache:'no-store'}),j=await r.json().catch(()=>null);
    if(!r.ok||j?.ok!==true)throw new Error(j?.error||`HTTP_${r.status}`);
    fullCache.set(id,{at:Date.now(),data:j});renderCard(card);
  }catch(e){console.warn('[NOMAD343 FULL ODDS]',id,e);renderCard(card)}finally{fullBusy.delete(id)}
}
function afterToggle(card){setTimeout(()=>{if(card?.getAttribute('aria-expanded')==='true')loadFull(card)},0)}
...
function start(){
  ...
  const mo=new MutationObserver(records=>{if(!lastSnapshot)return;const changed=records.some(r=>[...r.addedNodes].some(n=>n?.nodeType===1&&(n.matches?.('.match-card[data-match-id]')||n.querySelector?.('.match-card[data-match-id]'))));if(changed)setTimeout(decorate,0)});
  mo.observe(document.body,{childList:true,subtree:true});
  loadBoard();setInterval(loadBoard,POLL_MS);
  window.NOMAD343_FULL_ODDS_MAIN={version:VERSION,reload:id=>{const key=String(id);fullCache.delete(key);const card=document.querySelector(`.match-card[data-match-id="${CSS.escape(key)}"]`);if(card)loadFull(card)},refresh:loadBoard};
}
