(()=>{
'use strict';
const VERSION='343-full-odds-main-v6-complete-10book';
...
function rowKey(row){return norm(row?.slug??row?.bookmaker?.slug??row?.name??row?.bookmaker?.name??row?.provider?.name??row?.provider)}
function matchesBook(book,row){const key=rowKey(row);if(!key)return false;return book.aliases.some(alias=>{const a=norm(alias);return key===a||key.includes(a)||a.includes(key)})}
function providerBooks(payload){
  ...
  if(!raw.length){
    ...
    for(const root of roots){
      ...
      for(const book of BOOKS){
        const hit=entries.find(([k,v])=>v&&typeof v==='object'&&book.aliases.some(alias=>{const a=norm(alias),key=norm(k);return key===a||key.includes(a)||a.includes(key)}));
        if(hit)keyed.push({slug:book.slug,name:book.name,...hit[1]});
      }
  ...
  const found=[];
  for(const book of BOOKS){
    const row=raw.find(x=>matchesBook(book,x));
    if(!row)continue;
...
function noDataRow(kind){
  if(kind==='1X2')return'<div class="fom-price-row fom-no-data cols-1x2"><span>—</span><strong>NO DATA</strong><strong>—</strong><strong>—</strong></div>';
  return`<div class="fom-price-row fom-no-data ${kind==='AH'?'cols-ah':'cols-total'}"><span>—</span><strong>NO DATA</strong><strong>—</strong></div>`;
}
function marketHtml(marketKey,bookMarkets,card,fixtureId,prev,next){
  ...
  let withData=0;
  for(const book of BOOKS){
    const market=bookMarkets.get(book.slug);
    const priceRows=market?stages(market).map(([,label,value])=>stageRowHtml(kind,label,value,fixtureId,book.slug,marketKey,prev,next)).filter(Boolean):[];
    if(priceRows.length)withData+=1;
    bookRows.push(`<section class="fom-book${priceRows.length?'':' no-data'}" data-book="${esc(book.slug)}"><header><b>${esc(book.name)}</b><small>${esc(book.role)}</small></header>${priceRows.length?priceRows.join(''):noDataRow(kind)}</section>`);
  }
  const title=LABELS[marketKey]||String(marketKey).replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase());
  return `<section class="fom-market"><header class="fom-market-head"><div><b>${esc(title)}</b><small>${kind==='AH'?'Signed handicap shown on each team side':kind==='1X2'?'Home · Draw · Away':'Line + decimal price'}</small></div><span>${withData}/10 DATA · 10 BOOKS</span></header>${columnHead(kind,home,away)}<div class="fom-books">${bookRows.join('')}</div></section>`;
}
function sourceText(payload,books){if(payload?.fallbackOnly)return'BET365 FALLBACK';if(payload?.stale)return`${books.length}/10 RETURNED · STALE CACHE`;return`${books.length}/10 RETURNED · LIVE FEED`}
