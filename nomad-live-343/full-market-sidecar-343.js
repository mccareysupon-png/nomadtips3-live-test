...
function render(){const api=window.NOMAD343_DASHBOARD_V2;if(!api?.getFixture)return;document.querySelectorAll(ROW).forEach(row=>{const id=String(row.dataset.matchId||'');if(!id)return;const fixture=api.getFixture(id);if(!fixture)return;const entry=cache.get(id)||null;let side=row.nextElementSibling;if(!side?.classList?.contains(SIDECAR_CLASS)){side=document.createElement('div');side.className=SIDECAR_CLASS;side.dataset.forMatch=id;row.insertAdjacentElement('afterend',side)}const stamp=[entry?.fetchedAt,entry?.held,entry?.lastAttemptAt,fixture?.providerOddsFreshAt,JSON.stringify(producer||{}).length].join('|');if(side.dataset.stamp!==stamp){side.dataset.stamp=stamp;side.innerHTML=sidecarHtml(fixture,entry)}renderExpanded(id,fixture,entry)})}
