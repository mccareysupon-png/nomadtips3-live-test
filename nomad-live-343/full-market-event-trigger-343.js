(()=>{
'use strict';
const VERSION='343-full-market-event-trigger-v1';
...
async function trigger(id,reason){try{const r=await fetch(`${EVENT_API}?fixtureId=${encodeURIComponent(id)}&reason=${encodeURIComponent(reason)}&_=${Date.now()}`,{cache:'no-store'}),j=await r.json().catch(()=>null);if(!r.ok||j?.ok!==true)return;window.NOMAD343_FULL_ODDS_MAIN?.reload?.(id)}catch{}}
