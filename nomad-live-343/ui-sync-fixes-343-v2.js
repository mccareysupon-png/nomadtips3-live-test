...
function decorateFlow(svg,src){
  ...
  const events=eventsOf(src),current=currentMinute(src);if(!events.length||current===null||current<=0)return;
  const vb=svg.viewBox?.baseVal,w=vb?.width||1000;const left=42,right=18,usable=Math.max(1,w-left-right);
  const home=src?.home,away=src?.away,homePath=svg.querySelector('.expand-flow-line.home'),awayPath=svg.querySelector('.expand-flow-line.away');if(!homePath||!awayPath)return;
  const used=new Map();
...
function decorateSignalFlows(){document.querySelectorAll('.next-signal-card[data-next-card]').forEach(card=>{const svg=card.querySelector('.expand-flow-chart svg');if(!svg)return;const s=signalByFixture.get(String(card.dataset.nextCard||''));if(s)decorateFlow(svg,s)})}
...
window.addEventListener('ball46:signals-snapshot',e=>{const rows=Array.isArray(e.detail?.signals)?e.detail.signals:[];signalByFixture=new Map(rows.map(s=>[String(s?.fixtureId??s?.id??''),s]));syncSignalCount(rows);requestAnimationFrame(decorateSignalFlows)});
...
document.addEventListener('ball46:workspace-view',e=>{if(e.detail?.view==='signal')requestAnimationFrame(decorateSignalFlows)});
function init(){loadStatCountsOnce();const rows=window.NOMAD343_DASHBOARD_V2?.getSignals?.();if(Array.isArray(rows)){signalByFixture=new Map(rows.map(s=>[String(s?.fixtureId??s?.id??''),s]));syncSignalCount(rows);requestAnimationFrame(decorateSignalFlows)}}
