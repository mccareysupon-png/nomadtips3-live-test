...
function avgEntryOdds(rows){const values=(Array.isArray(rows)?rows:[]).map(s=>num(s?.odds)).filter(v=>v!==null&&v>1);if(!values.length)return null;return values.reduce((sum,v)=>sum+v,0)/values.length}
...
function render(j){marketDefs=j.markets||{};const rows=Array.isArray(j.rows)?j.rows:[];const avgOdds=avgEntryOdds(rows);set('[data-total]',j.total);set('[data-win]',j.win);set('[data-loss]',j.loss);set('[data-push]',j.push);set('[data-win-rate]',j.winRate===null?'—':`${j.winRate}%`);set('[data-avg-odds]',avgOdds===null?'—':avgOdds.toFixed(2));renderMarketSummary(j);const tbody=document.querySelector('[data-stat-body]');if(tbody)tbody.innerHTML=rows.length?rows.map((s,rowIndex)=>{const ordinal=rows.length-rowIndex,league=[s.league?.country,s.league?.name].filter(Boolean).join(' · '),m=entryMinute(s);return`<tr><td>${ordinal}</td><td>${esc(dt(s.createdAt))}</td><td><span class="match-name">${esc(s.home?.name||'—')} - ${esc(s.away?.name||'—')}</span><span class="sub">${esc(league||'—')}</span></td><td class="live-minute-343">${esc(show(m))}${m==null?'':"'"}</td><td><span class="match-name">${esc(label(s))}</span><span class="sub">${esc(s.period||'—')} · ${esc(s.bookmaker||'Bet365')}</span></td><td>${esc(selectionLabel(s))}</td><td>${lineCell(s)}</td><td class="odds">${esc(show(s.odds))}</td><td>${esc(s.bookmaker||'Bet365')}</td><td>${esc(score(s.entryScore??s.scoreAt))}</td><td>${esc(score(s.finalScore))}</td><td>${resultCell(s)}</td></tr>`}).join(''):'<tr><td colspan="12" class="empty">ยังไม่มีสัญญาณที่จบการแข่งขันและตัดสินผลแล้ว</td></tr>';const st=document.querySelector('[data-stat-state]');if(st)st.innerHTML=`<span class="pill"><i class="dot live"></i>Results · ALL MARKETS · ${esc(j.settlementRevision||'—')}${j.unresolved?` · UNRESOLVED ${esc(j.unresolved)}`:''}${j.halfWin||j.halfLoss?` · HALF ${j.halfWin||0}/${j.halfLoss||0}`:''}</span>`}
