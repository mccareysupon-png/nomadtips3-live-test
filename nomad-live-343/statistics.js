...
function render(j){marketDefs=j.markets||{};set('[data-total]',j.total);set('[data-win]',j.win);set('[data-loss]',j.loss);set('[data-win-rate]',j.winRate===null?'—':`${j.winRate}%`);renderMarketSummary(j);const rows=Array.isArray(j.rows)?j.rows:[];const tbody=document.querySelector('[data-stat-body]');if(tbody)tbody.innerHTML=rows.length?rows.map(s=>{const league=[s.league?.country,s.league?.name].filter(Boolean).join(' · '),m=entryMinute(s);return`<tr><td>${esc(dt(s.createdAt))}</td><td><span class="match-name">${esc(s.home?.name||'—')} - ${esc(s.away?.name||'—')}</span><span class="sub">${esc(league||'—')}</span></td><td class="live-minute-343">${esc(show(m))}${m==null?'':"'"}</td><td><span class="match-name">${esc(label(s))}</span><span class="sub">${esc(s.providerMarket||'—')} · ${esc(s.period||'—')}</span></td><td>${esc(show(s.selection))}</td><td>${esc(show(s.line))}</td><td class="odds">${esc(show(s.odds))}</td><td>${esc(s.bookmaker||'Bet365')}</td><td>${esc(score(s.entryScore??s.scoreAt))}</td><td>${esc(score(s.finalScore))}</td><td>${resultTag(s.result)}</td></tr>`}).join(''):'<tr><td colspan="11" class="empty">ยังไม่มีสัญญาณที่จบการแข่งขันและตัดสินผลแล้ว</td></tr>';const st=document.querySelector('[data-stat-state]');if(st)st.innerHTML=`<span class="pill"><i class="dot live"></i>Results · ALL MARKETS${j.halfWin||j.halfLoss?` · HALF ${j.halfWin||0}/${j.halfLoss||0}`:''}</span>`}
