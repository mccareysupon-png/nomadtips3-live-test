...
function sweepExpanded(){document.querySelectorAll('.match-card[data-match-id][aria-expanded="true"] [data-full-odds-main] .fom-board').forEach(board=>{board.classList.remove('fom-refresh-sweep');void board.offsetWidth;board.classList.add('fom-refresh-sweep')})}
async function load(){if(busy||document.visibilityState==='hidden')return;busy=true;try{install();const r=await fetch(`${API}?_=${Date.now()}`,{cache:'no-store'}),j=await r.json().catch(()=>null);if(!r.ok||j?.ok!==true||!Array.isArray(j.fixtures))return;cache.clear();for(const row of j.fixtures){const id=String(row?.fixtureId||'').trim();if(id&&row?.fullOdds)cache.set(id,row)}lastLoadedAt=Date.now();repaintExpanded();sweepExpanded()}catch{}finally{busy=false}}
